...
def run_for_milliseconds_with_shift(key_code, duration_ms):
    start_time = time.time()
    while (time.time() - start_time) * 1000 < duration_ms:
        PressKey(key_mapping['Shift'])  # 按住Shift键
        PressKey(key_code)  # 同时按下W键
        time.sleep(0.01)
    ReleaseKey(key_code)  # 释放W键
    ReleaseKey(key_mapping['Shift'])  # 释放Shift键
...
if kuang==3:
    # 向前跑 7.3秒
    PressKey(key_mapping['Shift'])  # 按住Shift键
    run_for_milliseconds(key_mapping['W'], 7300)
    ReleaseKey(key_mapping['Shift'])  # 释放Shift键
    time.sleep(0.5)
    # 向左转
